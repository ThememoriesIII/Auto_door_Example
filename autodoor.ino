...
#define Max_dis 10 //ค่าระยะสูงสุดที่คนเดินเข้ามา
#define Low_dis 1 //ค่าระยะต่ำสุดที่คนเดืนเข้า
...
void door_control()
{
    ...
    if((messur_dist>Low_dis&&messur_dist<Max_dis)&&door==doorclose)
    {
      openAdoor();
      delay(100);
      while(digitalRead(Sdoor_close)==LOW&&digitalRead(Sdoor_open)==LOW)
      {
        doorTime = millis();
      }
    }
...
void sw_door()
{
  ...
  */
  door = (digitalRead(Sdoor_close)==LOW&&digitalRead(Sdoor_open)==HIGH)?doorclose:dooropen;
}
